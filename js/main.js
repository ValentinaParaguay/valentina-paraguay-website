...
(function () {
  ...
  /* ----------------------------------------------------------
     6b. FORMSPREE FORMS – AJAX submit as JSON
  ---------------------------------------------------------- */
  document.querySelectorAll('form[data-formspree]').forEach(function (fsForm) {
    const status = fsForm.querySelector('.formspree-status');
    ...
    fsForm.addEventListener('submit', function (e) {
      ...
      const data = {};
      new FormData(fsForm).forEach(function (value, key) { data[key] = value; });
      ...
      fetch(fsForm.action, {
        ...
        .then(function (result) {
          if (result.ok) {
            fsForm.reset();
            if (status) {
              status.textContent = '¡Listo! Te sumamos a la lista. Pronto sabrás de mí.';
              status.style.color = '';
            }
          } else {
            const msg = (result.body && result.body.errors && result.body.errors[0] && result.body.errors[0].message)
              || 'No pudimos enviar el formulario. Intentá de nuevo.';
            if (status) {
        ...
        .catch(function () {
          if (status) {
            status.textContent = 'Error de conexión. Probá de nuevo en unos segundos.';
            status.style.color = '#E53E3E';
          }
        })
        .finally(function () {
